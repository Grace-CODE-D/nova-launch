...
pub fn emit_admin_transfer(env: &Env, old_admin: &Address, new_admin: &Address) {
    env.events()
        .publish((symbol_short!("adm_xfer"),), (old_admin, new_admin));
}

/// Emit admin proposed event
///
/// Emitted when current admin proposes a new admin (two-step transfer)
...
pub fn emit_fees_updated(env: &Env, base_fee: i128, metadata_fee: i128) {
    env.events()
        .publish((symbol_short!("fee_upd"),), (base_fee, metadata_fee));
}
